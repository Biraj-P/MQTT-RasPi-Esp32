...
void DEV_I2C_Read_nByte(uint8_t addr, uint8_t reg, uint8_t *buf, uint16_t len) {
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (addr << 1) | I2C_MASTER_WRITE, true);
    i2c_master_write_byte(cmd, reg, true);
...
void QMI8658_read_xyz(float acc[3], float gyro[3], unsigned int *tim_count)
{
	...
	// mg
	acc[AXIS_X] = (float)(raw_acc_xyz[AXIS_X] * 1.0f) / acc_lsb_div;
	acc[AXIS_Y] = (float)(raw_acc_xyz[AXIS_Y] * 1.0f) / acc_lsb_div;
	acc[AXIS_Z] = (float)(raw_acc_xyz[AXIS_Z] * 1.0f) / acc_lsb_div;
#else
	// m/s2
...
void QMI8658_read_ae(float quat[4], float velocity[3])
{
	...
	short raw_q_xyz[4] = {0,0,0,0};
	short raw_v_xyz[3] = {0,0,0};
...
unsigned char QMI8658_init(void)
{
    ...
    //   printf("QMI8658Register_WhoAmI = ");
    //   printf(QMI8658_chip_id);
//			QMI8658_printf("QMI8658Register_WhoAmI = 0x%x\n", QMI8658_chip_id);
	...
	if (QMI8658_chip_id == 0x05)
	{
    // printf("QMI8658_init slave = ");
    // printf(QMI8658_slave_addr);
    // printf("nQMI8658Register_WhoAmI = ");
    // printf(QMI8658_chip_id);
    // printf(" ");
    // printf(QMI8658_revision_id);
